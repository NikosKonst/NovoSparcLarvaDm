  ...
  files <- list.files("collected_files", pattern = "_column\\.csv$", full.names = TRUE)
  
  data_list <- lapply(files, function(f) {
    read.csv(f, stringsAsFactors = FALSE)
  })
  
  final_table <- do.call(cbind, data_list)
  
  norm_table <- as.data.frame(
    apply(final_table, 2, function(x) {
      (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE))
    })
  )
  
  norm_table_final<-cbind(coords,norm_table)
  ...
  reconstruction_pre<-norm_table_final[, c(1:3, which(names(norm_table_final) == gene_of_interest))]
  ...
  write.table(results, file = "precision_recall_leave_one_out.txt", quote=F, sep = "\t")
  ...
  p <- ggplot(results_long, aes(x = metric, y = value, fill = metric)) +
    ...
    labs(title = "Precision and Recall Across Marker Genes - LOOV",
         x = "", y = "Value") +
  ...
  ggsave("precision_recall_leave_one_out.pdf", p, width = 4, height = 8)
